...
# process the rack (segment 0)
rack_mask = masks_sorted[0]["segmentation"]
rack_pcd_observed = create_point_cloud_from_rgbd(
    rgb=rgb,
    depth=depth,
    K=K,
...
scales_to_try = [0.15, 1.0, 2.0, 3.0, 4.0, 4.77, 5.0, 6.0]
rot_to_try = [
    ("no rotation", [0, 0, 0]),
    ("90° X", [90, 0, 0]),
    ("90° Y", [0, 90, 0]),
...
print(f"Testing {len(scales_to_try)} scales × {len(rot_to_try)} rotations...")

# loop to test
for scale in scales_to_try:
    for rot_name, rot_angles in rot_to_try:
        # prepare the model
        model = o3d.geometry.TriangleMesh(rack_mesh)
        ...
        # run ICP
        world_T_model, fitness, rmse = estimator.estimate_pose(model_pcd, rack_pcd_observed)
        ...
        if fitness > best_fitness:
            ...
            print(f"New best: scale={scale:.2f}, rot={rot_name:12s}, fitness={fitness:.4f}, RMSE={rmse:.6f}")
